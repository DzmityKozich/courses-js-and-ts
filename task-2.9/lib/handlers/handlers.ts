...
export const getAllHandler: RequestHandler = async (_, res) => {
	const todoList = await (await repository()).getAll();
	res.status(200).send(todoList);
};

export const getByIdHandler: RequestHandler<{ id: string }> = async (req, res) => {
	const todo = await (await repository()).getById(req.params.id);
	if (todo) {
...
export const createHandler: RequestHandler = async (req, res) => {
	try {
		const todo = await (await repository()).create(req.body);
		res.status(200).send(todo);
	} catch (error) {
		console.log(error);
...
export const updateHandler: RequestHandler = async (req, res) => {
	try {
		const todo = await (await repository()).update(req.body);
		res.status(200).send(todo);
	} catch (error) {
		console.log(error);
		res.status(400).send('Invalid data!');
...
export const deleteHandler: RequestHandler<{ id: string }> = async (req, res) => {
	const result = await (await repository()).delete(req.params.id);
	if (result) {
