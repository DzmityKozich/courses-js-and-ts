...
function getLocationSearchURL(location: string): string {
	return `${import.meta.env.VITE_LOCATION_SEARCH_URL}?q=${location}&limit=2&appid=${API_KEY}`;
}

function getForecastSearchURL(lat: Cord, lon: Cord, cnt: number = 40): string {
	return `${import.meta.env.VITE_FORECAST_SEARCH_URL}?lat=${lat}&lon=${lon}&appid=${API_KEY}&units=metric&cnt=${cnt}`;
}

function getCurrentForecastSearchURL(lat: Cord, lon: Cord) {
	return `${import.meta.env.VITE_CURRENT_FORECAST_SEARCH_URL}?lat=${lat}&lon=${lon}&appid=${API_KEY}&units=metric`;
}
...
export class ForecastSearchService {
	...
	private splitOnDayWeather(res: HourForecast[], sunrise: dayjs.Dayjs, sunset: dayjs.Dayjs): DayWeather[] {
		...
		return Object.entries(dailyForecast)
			.filter(([_, hf]) => hf.length)
			.map(([key, hf]) => new DayWeather(+key, hf, sunrise, sunset));
	}
